    ...
    if ($cd->{gen_args}{subcommands}) {
        ...
        for my $o (sort keys %$specmetas) {
            ...
            if ($co eq 'help') {
                ...
                push @l, 'if (!length $sc_name && @ARGV) { $sc_name = $ARGV[0] } ';
                push @l, 'if (!length $sc_name && $_[2]{_non_options_argv} && @{ $_[2]{_non_options_argv} }) { $sc_name = $_[2]{_non_options_argv}[0] } ';
        ...
        push @l, 'if (!length $_pci_r->{subcommand_name} && @{ $res->{subcommand} // [] }) { $_pci_r->{subcommand_name} = $res->{subcommand}[0] }', "\n";
